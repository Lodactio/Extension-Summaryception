...
function debugVisibleTurns(chat, store) {
   ...
   const unghosteredSummarized = visibleIndices.filter(
      (idx) => idx <= store.summarizedUpTo,
   );
   if (unghosteredSummarized.length > 0) {
      ...
      trace(
         "  First 5 unghostered summarized indices:",
         unghosteredSummarized.slice(0, 5),
      );
   }
   trace("=== END DEBUG ===");
}
...
async function ghostMessage(messageIndex) {
   ...
   log(
      `Ghosted message at index ${messageIndex}${s.disableGhosting ? " (hiding disabled)" : ""}`,
   );
}
...
async function unghostAllMessages() {
   ...
   const toUnhide =
      store.ghostedIndices && store.ghostedIndices.length > 0
   ...
   log(
      `Unghosted ${toUnhide.length} messages (only Summaryception-hidden ones)`,
   );
}
...
async function repairIfBranched() {
   ...
   if (store.summarizedUpTo >= chatLength) {
      ...
      if (store.ghostedIndices) {
         store.ghostedIndices = store.ghostedIndices.filter(
            (idx) => idx < chatLength,
         );
      }
...
async function callSummarizer(storyTxt, contextStr) {
   ...
   const isDefaultMode =
      !s.connectionSource || s.connectionSource === "default";
   ...
   try {
      for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
         ...
         if (signal.aborted) {
            ...
            toastr.warning("Summarization aborted.", "Summaryception", {
               timeOut: 3000,
            });
            return "";
         }
         ...
         try {
            ...
            trace(`  About to call sendSummarizerRequest with:`, {
               connectionSource: s.connectionSource,
               summarizerSystemPrompt: s.summarizerSystemPrompt?.substring(
                  0,
                  50,
               ),
            ...
            trace(
               "  sendSummarizerRequest returned:",
            ...
            if (attempt >= RETRY_CONFIG.maxRetries) {
               ...
               console.error(
                  LOG_PREFIX,
                  `All ${RETRY_CONFIG.maxRetries} retries exhausted.`,
               );
               break;
            }
...
async function maybeSummarizeTurns() {
   ...
   const visibleTurns = allAssistantTurns.filter(
      (t) => !chat[t.index].extra?.sc_ghosted,
   );

   log(
      `Visible assistant turns: ${visibleTurns.length}, limit: ${s.verbatimTurns}`,
   );
   ...
   if (overflow > backlogThreshold && !catchupDismissed) {
      ...
      if (choice === "skip") {
         ...
         if (cutoff) {
            ...
            log(
               `Skipped backlog. summarizedUpTo set to ${store.summarizedUpTo}`,
            );
         }
   ...
   const remaining = getAssistantTurns(chat).filter(
      (t) => !chat[t.index].extra?.sc_ghosted,
   );
   if (
...
async function summarizeOneBatch(visibleTurns) {
   ...
   const eligibleTurns = visibleTurns.filter(
      (t) => t.index > store.summarizedUpTo,
   );
   ...
   try {
      ...
      log(
         `Summarizing ${batch.length} assistant turns (indices ${startIdx}–${endIdx})`,
      );
      ...
      if (passageStart > endIdx) {
         ...
         trace(
            "<<< EXITING summarizeOneBatch - PASSAGE START GREATER THAN END",
         );
         return false;
      }
      ...
      if (!summary) {
         log(
            "Summarization failed for batch, leaving turns intact for next attempt.",
         );
...
async function summarizeOneBatchFromTurns(visibleTurns) {
   ...
   try {
      const storyTxt = buildPassageFromRange(chat, passageStart, endIdx);
      trace(
         "  buildPassageFromRange returned, length:",
      ...
      if (!storyTxt.trim()) {
         ...
         trace(
            "  This suggests all messages in range [" +
               passageStart +
               ", " +
               endIdx +
               "] are hidden or empty",
      ...
      const contextStr = buildFullContext(0);
      trace(
         "  buildFullContext returned, length:",
         contextStr?.length ?? "UNDEFINED",
      );

      trace("  About to call callSummarizer...");
      const summary = await callSummarizer(storyTxt, contextStr);
      trace(
         "  callSummarizer returned, length:",
...
async function showCatchupDialog(overflowCount, estimatedCalls) {
   return new Promise((resolve) => {
      ...
      overlay
         .querySelector("#sc_catchup_full")
         .addEventListener("click", () => {
            overlay.remove();
            resolve("catchup");
         });
      overlay
         .querySelector("#sc_catchup_skip")
         .addEventListener("click", () => {
            overlay.remove();
            resolve("skip");
         });
      overlay
         .querySelector("#sc_catchup_partial")
         .addEventListener("click", () => {
            overlay.remove();
            resolve("partial");
         });
...
async function maybePromoteLayer(layerIndex) {
   ...
   log(
      `Layer ${layerIndex}: ${layer.length} snippets > limit ${s.snippetsPerLayer} → promoting`,
   );
...
function updateInjection() {
   try {
      ...
      // 1. Manual Mode: Clear legacy extension prompt slot, let macro handle delivery
      ...
      // 2. Automatic Mode: Check if extension is disabled
      if (!s.enabled) {
         if (_lastInjected !== "") {
            setExtensionPrompt(MODULE_NAME, "", 0, 0, false, 0);
            _lastInjected = "";
      ...
      // 3. Automatic Mode: Assemble block and inject at top of chatlog
      const summaryBlock = assembleSummaryBlock();
      if (summaryBlock === _lastInjected) return;

      // position 1 (IN_CHAT), depth 100 (clamps to top of chat), scan false, role 0 (SYSTEM)
      setExtensionPrompt(MODULE_NAME, summaryBlock || "", 1, 100, false, 0);
      _lastInjected = summaryBlock || "";
      log(
         `Injection updated (Automatic): ${(summaryBlock || "").length} chars`,
      );
   } catch (e) {
...
function registerSlashCommands() {
   try {
      ...
      if (!ctx.SlashCommandParser?.addCommandObject || !ctx.SlashCommand) {
         log(
            "SlashCommandParser not available, skipping command registration.",
         );
         return;
      }
...
function updateUI() {
   try {
      ...
      if (
         !store.layers?.length ||
         store.layers.every((l) => !l || l.length === 0)
      ) {
         statsHtml =
            '<div class="sc-layer-stat sc-muted">No summaries yet for this chat.</div>';
      }
...
function updateSnippetBrowser() {
   ...
   if (!store.layers || store.layers.every((l) => !l || l.length === 0)) {
      ...
      for (let i = store.layers.length - 1; i >= 0; i--) {
         ...
         if (!layer || layer.length === 0) continue;
         const label =
            i === 0 ? "Layer 0 (Turn Summaries)" : `Layer ${i} (Meta-Summary)`;
   ...
   $(".sc-snippet-redo")
      ...
      .on("click", async function () {
         const layerIdx = parseInt(
            $(this).closest(".sc-snippet").data("layer"),
         );
         const snippetIdx = parseInt(
            $(this).closest(".sc-snippet").data("idx"),
         );
         const store = getChatStore();
         const layer = store.layers[layerIdx];
         if (!layer || !layer[snippetIdx]) return;
         ...
         if (isSummarizing) {
            toastr.warning(
               "Already summarizing. Please wait.",
               "Summaryception",
            );
            return;
         }
         ...
         if (
            !confirm(`Regenerate summary for turns ${rangeStart}–${rangeEnd}?`)
         )
            return;
         ...
         try {
            ...
            if (!storyTxt.trim()) {
               toastr.error(
                  "Source turns are empty — cannot regenerate.",
                  "Summaryception",
               );
               return;
            }
            ...
            const contextStr =
               contextParts.length > 0 ? contextParts.join(" ") : "(none yet)";
            ...
            if (!newSummary) {
               toastr.error(
                  "Regeneration failed — original snippet kept.",
                  "Summaryception",
               );
               return;
            }
...
function bindUIEvents() {
   ...
   $("#sc_injection_mode").on("change", function () {
      const s = getSettings();
   ...
   $(document).on("click", "#sc_repair", async function () {
      ...
      if (repaired > 0) {
         ...
         toastr.info("No orphaned messages found.", "Summaryception", {
            timeOut: 3000,
         });
      }
   });
   ...
   $(document).on("click", "#sc_clear_memory", async function () {
      if (
         !confirm(
            "Clear ALL Summaryception memory for this chat and unghost all messages?",
         )
      )
         return;
      ...
      try {
         ...
         console.error(
            LOG_PREFIX,
            "Error during unghost (continuing with clear):",
            e,
         );
         toastr.warning(
   ...
   $(document).on("click", "#sc_force_summarize", async function () {
      ...
      try {
         ...
         const overflow = visibleTurns.length - s.verbatimTurns;
         toastr.info(
            `${overflow} turns to process. Starting...`,
            "Summaryception",
            { timeOut: 2000 },
         );
   ...
   $(document).on("click", "#sc_stop_summarize", function () {
      ...
      abortSummarization();
      toastr.warning(
         "Summarization stopped. Progress has been saved.",
         "Summaryception",
         { timeOut: 4000 },
      );
   ...
   $(document).on("click", "#sc_custom_prompt_load", function () {
      ...
      toastr.success(`Loaded prompt "${name}".`, "Summaryception", {
         timeOut: 2000,
      });
   });
   ...
   $(document).on("click", "#sc_custom_prompt_delete_slot", function () {
      ...
      updateCustomPromptSlots();
      toastr.info(`Prompt "${name}" deleted.`, "Summaryception", {
         timeOut: 2000,
      });
   });
   ...
   $(document).on("click", "#sc_custom_prompt_export", function () {
      ...
      if (!promptText.trim()) {
         toastr.warning(
            "Prompt is empty — nothing to export.",
            "Summaryception",
         );
         return;
      }
...
function initConnectionUI() {
   ...
   const sourceSelect = document.getElementById(
      "summaryception_connection_source",
   );
   if (sourceSelect) {
   ...
   const profileSelect = document.getElementById(
      "summaryception_connection_profile",
   );
   if (profileSelect) {
   ...
   const ollamaRefresh = document.getElementById(
      "summaryception_ollama_refresh",
   );
   if (ollamaRefresh) {
   ...
   // ── OpenAI Max Tokens ──
   const openaiMaxTokens = document.getElementById(
...
async function refreshOllamaModels() {
   ...
   try {
      ...
      toastr.error(
         `Failed to fetch Ollama models: ${error.message}`,
         "Summaryception",
      );
   }
}
...
function showConnectionStatus(type, message) {
   const container = document.getElementById(
      "summaryception_connection_status",
   );
   const icon = document.getElementById(
      "summaryception_connection_status_icon",
   );
   const text = document.getElementById(
      "summaryception_connection_status_text",
   );
...
async function fetchProfilesFallback(selectElement, currentValue) {
   try {
      ...
      if (!response.ok) {
         console.warn(
            "[Summaryception] Could not fetch connection profiles from API",
         );
         return;
      }
      ...
      console.warn(
         "[Summaryception] Could not fetch connection profiles:",
         error,
      );
   }
}
...
(async function init() {
   ...
   if (ctx.registerMacro) {
      ...
      log(
         "Warning: SillyTavern.getContext().registerMacro not found. Macro mode unavailable.",
      );
   }
